...
def eight_stairs(result_list):
    ...
    while(len(result_list)<19):#列が19個出るまで続ける。
        
        data_1=random.randrange(1,3,1)
        total+=data_1
        
        if(data_1==2 and data_2==2):
            data_1=1#改良型：酒田先生の授業で言及してたやつ
            data_2=1 #2が連続できたとき2を1に置き換える。
        list.append(data_1)
        data_2=data_1
...
def kaidan(total_step, before_step, process):
   ...
   if before_step == 1:
        ...
        kaidan(total_step, 2, process)    →連続で関数を続けることで同時に探索していく。
                                          →今まではreturnの後に関数を付けることで再帰だと思ってたけど必ずそうする必要はなかった。
